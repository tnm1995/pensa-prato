      ...
      {Array.from({ length: 40 }).map((_, i) => (
        <motion.div
          ...
          className={`absolute w-3 h-3 rounded-sm ${
            ['bg-amber-400', 'bg-emerald-400', 'bg-rose-400', 'bg-blue-400', 'bg-white'][i % 5]
          }`}
        />
      ))}
    </div>
...
export const RecipeDetail: React.FC<RecipeDetailProps> = ({ 
  ...
  // Conversor de decimal para frações culinárias humanas
  const formatCulinaryNumber = (num: number): string => {
    ...
    // Se não for uma fração comum, usa decimal arredondado
  ...
  const scaleText = (text: string, originalServings: number, newServings: number): string => {
    ...
    // Regex aprimorada para capturar números, frações e unidades
    // Captura: "1", "1/2", "1,5", "0.5", "1 1/2"
    return text.replace(/(\d+\s+\d+\/\d+|\d+\/\d+|\d+[.,]\d+|\d+)(\s*)([a-zA-ZÀ-ÿ°º]+)?/g, (match, numberStr, space, unit) => {
        ...
        // Se for uma unidade de tempo ou temperatura, não escala
        if (forbiddenUnits.some(u => lowerUnit.startsWith(u))) return match;

        // Se for "Passo 1", "Passo 2", etc, não escala
        if (text.toLowerCase().includes('passo') && match.length < 5) return match;

        let value: number = 0;
        
        try {
            // Caso 1: Número Misto "1 1/2"
            if (numberStr.includes(' ') && numberStr.includes('/')) {
                const [w, f] = numberStr.split(/\s+/);
                const [n, d] = f.split('/').map(Number);
                value = Number(w) + (n / d);
            } 
            // Caso 2: Fração Pura "1/2"
            else if (numberStr.includes('/')) {
                const [n, d] = numberStr.split('/').map(Number);
                value = n / d;
            } 
            // Caso 3: Decimal ou Inteiro "1.5" ou "1,5"
            else {
        ...
        return (
            ...
            >
                ...
                <motion.div 
                  ...
                  className="relative bg-white p-8 pt-16 rounded-[3rem] w-full max-w-sm text-center shadow-2xl z-10"
                >
                    <div className="absolute -top-14 left-1/2 -translate-x-1/2">
                        <motion.div 
                          ...
                          className="w-28 h-28 bg-amber-400 rounded-[2.5rem] flex items-center justify-center shadow-xl border-4 border-white rotate-6"
                        >
                    ...
                    <p className="text-stone-500 mb-8 text-sm font-medium">Você preparou o prato com perfeição. O Chef está orgulhoso!</p>
                    
                    <div className="bg-stone-50 rounded-3xl p-6 mb-8 border border-stone-100">
                        <p className="text-[10px] font-black text-stone-400 uppercase tracking-[0.2em] mb-4">Dê sua nota para a IA</p>
                        <div className="flex justify-center gap-2">
                            {[1, 2, 3, 4, 5].map((s) => (
                                <button key={s} onClick={() => { setRating(s); onRate(s); }}>
                                  <Star className={`w-8 h-8 transition-all ${rating >= s ? 'fill-amber-400 text-amber-400 scale-110' : 'text-stone-200 hover:text-stone-300'}`} />
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <button 
                      ...
                      className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-black py-5 rounded-[2rem] shadow-xl shadow-emerald-100 transition-all flex items-center justify-center gap-2 group active:scale-95"
                    >
                      Concluir Receita <Sparkles className="w-5 h-5 group-hover:animate-spin" />
                    </button>
                </motion.div>
                ...
                >
                    <p className="text-emerald-200 text-[10px] font-bold uppercase tracking-widest">+0.4kg de comida salva</p>
    ...
    return (
      <div className="fixed inset-0 bg-neutral-950 z-[100] flex flex-col text-white overflow-hidden font-['Sora']">
        <div className="flex items-center justify-between p-6 shrink-0">
          ...
          <button 
            onClick={() => { window.speechSynthesis.cancel(); setCookingMode(false); }} 
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-6 text-center overflow-y-auto min-h-0">
            <p className="text-emerald-500 text-[10px] font-black uppercase tracking-[0.2em] mb-4 shrink-0">
            ...
            <div className="max-w-md w-full mb-8">
                <AnimatePresence mode="wait">
                  <motion.div
                    ...
                    exit={{ opacity: 0, x: -20 }}
                    className="text-2xl sm:text-3xl font-medium leading-tight text-white px-4"
                  >
            ...
            {timerDuration > 0 && (
                <div className="bg-neutral-900/80 rounded-[2.5rem] p-6 border border-neutral-800 w-full max-w-sm flex items-center justify-between shrink-0 mb-4 shadow-xl">
                    <div className="text-left">
                        <p className="text-[10px] uppercase font-bold text-neutral-500 mb-1">Timer Automático</p>
                    ...
                    <button 
                        ...
                        className={`px-8 py-3 rounded-2xl font-bold transition-all active:scale-95 ${isTimerRunning ? 'bg-neutral-700 text-white' : 'bg-emerald-600 text-white shadow-lg shadow-emerald-900/20'}`}
                    >
        ...
        <div className="bg-neutral-900 px-6 pt-6 pb-8 sm:pb-10 rounded-t-[3rem] border-t border-neutral-800 flex justify-center gap-4 shrink-0 shadow-[0_-10px_40px_rgba(0,0,0,0.4)]" style={{ paddingBottom: 'calc(env(safe-area-inset-bottom, 24px) + 16px)' }}>
            <button 
                ...
                className="flex-1 p-5 bg-neutral-800 disabled:opacity-20 rounded-[2rem] text-gray-400 flex items-center justify-center transition-all active:scale-95"
            >
                <SkipBack className="w-6 h-6" />
            </button>
            <button 
                onClick={() => isSpeaking ? window.speechSynthesis.cancel() : speakStep(currentStep)} 
                className={`flex-1 p-5 rounded-[2rem] flex items-center justify-center transition-all active:scale-95 ${isSpeaking ? 'bg-amber-500 shadow-lg shadow-amber-900/20' : 'bg-neutral-800'}`}
            >
            ...
            >
                {currentStep === recipe.instructions.length - 1 ? 'Concluir' : <SkipForward className="w-6 h-6" />}
            </button>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-white pb-40 font-['Sora']">
      <div className={`pt-12 pb-8 px-6 rounded-b-[3.5rem] shadow-lg relative overflow-hidden text-white ${isExplore ? 'h-80' : 'bg-emerald-600'}`}>
        ...
        <div className="flex justify-between items-center relative z-10 mb-20 md:mb-6">
            <button onClick={onBack} className="p-3 bg-white/20 backdrop-blur-md rounded-2xl active:scale-90 transition-transform">
                <ArrowLeft className="w-6 h-6" />
            </button>
            <button onClick={onToggleFavorite} className="p-3 bg-white/20 backdrop-blur-md rounded-2xl active:scale-90 transition-transform">
        ...
        <div className="relative z-10 mt-auto">
            ...
            <div className="flex gap-3 text-emerald-50 text-xs font-bold">
                <span className="bg-black/30 px-3.5 py-2 rounded-2xl backdrop-blur-md flex gap-2 items-center border border-white/10">
                    <Clock className="w-4 h-4" /> {recipe.time_minutes} min
                </span>
                <span className="bg-black/30 px-3.5 py-2 rounded-2xl backdrop-blur-md flex gap-2 items-center border border-white/10">
                    <Users className="w-4 h-4" /> {desiredServings} pessoas
                </span>
            </div>
        </div>
      </div>
      ...
      <div className="px-6 py-10 space-y-12 -mt-6 relative z-20">
        ...
        <div className="bg-white rounded-[2.5rem] p-6 flex justify-between border border-stone-100 shadow-xl shadow-stone-200/40 items-center">
            <div className="flex gap-4 items-center">
                <div className="p-3.5 bg-emerald-50 rounded-[1.5rem] text-emerald-600 shadow-sm">
            ...
            <div className="flex items-center gap-1 bg-stone-50 rounded-2xl p-1 border border-stone-100">
                <button 
                  ...
                  className="p-3 text-stone-400 hover:text-emerald-600 active:bg-white rounded-xl transition-all"
                >
        ...
        <section>
          <div className="flex items-center justify-between mb-8">
            ...
            <span className="text-[10px] font-bold text-stone-400 uppercase">Valores calculados</span>
          </div>
            ...
            {[...recipe.used_ingredients, ...recipe.missing_ingredients].map((ing, i) => {
                ...
                return (
                    <motion.li 
                      ...
                      className={`flex items-center justify-between gap-4 p-5 rounded-3xl border transition-all ${isMissing ? 'bg-stone-50 border-stone-100 shadow-sm' : 'bg-emerald-50/20 border-emerald-100'}`}
                    >
                        <div className="flex gap-4 items-center flex-1">
                            ...
                            <span className={`text-sm font-semibold leading-snug ${isMissing ? 'text-stone-500 italic' : 'text-stone-700'}`}>
                                {scaled}
                            </span>
                        </div>
                        ...
                        {isMissing && (
                            alreadyInList ? (
                                ...
                                <button 
                                    ...
                                    className="p-2.5 bg-orange-100 text-orange-600 rounded-xl hover:bg-orange-200 transition-colors shadow-sm active:scale-90"
           ...
           <div className="space-y-10 relative pl-4">
              ...
              {recipe.instructions.map((step, i) => (
                <div key={i} className="relative pl-12">
                  <div className="absolute left-0 top-0 w-9 h-9 rounded-[1.2rem] bg-white text-emerald-600 font-black flex items-center justify-center text-xs border border-stone-100 shadow-md z-10">{i + 1}</div>
                  <div className="bg-stone-50/40 p-6 rounded-[2.5rem] border border-stone-100 shadow-sm group hover:bg-white hover:shadow-xl transition-all duration-500">
      ...
      <div className="fixed bottom-0 left-0 right-0 p-6 z-[80] bg-gradient-to-t from-white via-white/95 to-transparent pt-12" style={{ paddingBottom: 'calc(env(safe-area-inset-bottom, 24px) + 24px)' }}>
        <button 
            ...
            className="w-full bg-stone-900 hover:bg-black text-white font-black py-6 rounded-[2.5rem] shadow-2xl shadow-stone-400 flex justify-center items-center gap-4 active:scale-95 transition-all transform group overflow-hidden"
        >
